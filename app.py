...
# Title
st.title("Historico de Precios del Mercado de Autos de Segunda Mano")
...
if selected_make and selected_make != "":
    ...
    if selected_model and selected_model != "":
        ...
        if not df.empty:
            # Show km filter
            km_options = [""] + sorted(df['km_range'].unique().tolist())
            selected_km = st.selectbox("Selecciona el rango de km:", options=km_options, index=0)

            # Filter in memory if the user selects a km
            if selected_km and selected_km != "":
                filtered_df = df[df['km_range'] == selected_km]
            else:
                filtered_df = (
